...
def _find_engine_root() -> Path:
    ...
    for parent in script_path.parents:
        if (parent / "src").is_dir() and (parent / "pyproject.toml").exists():
            return parent
        candidate = parent / "apps" / "ai-engine"
        if (candidate / "src").is_dir() and (candidate / "pyproject.toml").exists():
            return candidate
    raise RuntimeError("Could not find apps/ai-engine root for src imports")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--tokenizer",  default=None, metavar="PATH",
                   help="Tokenizer directory to bundle with the model. "
                        "If omitted, auto-detects when exactly one tokenizer exists.")
...
def resolve_tokenizer(tokenizer_arg: str | None) -> Path:
    """Resolve an explicit tokenizer path or auto-detect one local tokenizer."""
    if tokenizer_arg:
        return Path(tokenizer_arg).resolve()

    tokenizers_root = ENGINE_ROOT / "storage" / "tokenizers"
    candidates = [
        path for path in tokenizers_root.iterdir()
        if path.is_dir() and (path / "vocab.json").exists() and (path / "tokenizer_config.json").exists()
    ] if tokenizers_root.exists() else []

    if len(candidates) == 1:
        print(f"  Auto-detected tokenizer: {candidates[0]}")
        return candidates[0].resolve()

    if not candidates:
        sys.exit(
            "  ERROR: --tokenizer is required because no tokenizer was found under "
            f"{tokenizers_root}"
        )

    sys.exit(
        "  ERROR: --tokenizer is required because multiple tokenizers were found:\n"
...
def infer_config(state_dict: dict, model_type: str | None, seq_len_override: int | None) -> dict:
    ...
    # num_heads: infer from RoPE buffers when available. q_proj is usually
    # [d_model, d_model], so it cannot tell us how the model split heads.
    num_heads = 12  # safe default for gpt-small
    for k in state_dict:
        if k.endswith("rope.cos_table"):
            head_dim = state_dict[k].shape[1]
            if head_dim > 0 and d_model % head_dim == 0:
                num_heads = max(1, d_model // head_dim)
                break
    else:
        for k in state_dict:
            if "q_proj" in k and "weight" in k:
                head_dim = 64  # common GPT default
...
def validate_tokenizer(tokenizer_dir: Path) -> None:
    ...
    with open(tokenizer_dir / "tokenizer_config.json", encoding="utf-8") as f:
        tok_cfg = json.load(f)
...
def run_evaluation_gate(
    ...
    if eval_data and eval_data.exists():
        ...
        try:
            ...
            with open("/tmp/is_promote_eval.json", encoding="utf-8") as f:
